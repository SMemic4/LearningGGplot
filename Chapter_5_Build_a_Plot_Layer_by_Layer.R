...
# For example, smoother calculates the smoothed mean of y, conditional on x
...
# There are two ways to use these functions. Either add a stat_() function and override the default geom, or add a geom_function and override the default stat
...
###############################################################################################################################################################################################
# Internally, a stat takes a data frame as input and returns a data frame as output, and so a stat can add new variables to the original dataset. It is possible to map aesthetics to these new variables.
...
# For example, the default histogram geom assigns the height of the bars to the number of observations (count), but a more traditional histogram can use the density stat instead. To refer to a generated variable like density, ".." must surround the name
# This prevents confusion in the case the original dataset includes a variable with the same name as a generated variable, and makes it clear to any readers of the code that a variable is generated by a stat.
...
ggplot(diamonds, aes(price)) + geom_histogram(aes(y = ..density..), binwidth = 500) # The "..density.." notation was deprecated in the lastest ggplot update. It still works but after_stat() is used instead
ggplot(diamonds, aes(price)) + geom_histogram(aes(y = after_stat(density)), binwidth = 500) # The y axis of the graphs using the density plot uses an exponential scale 

# This technique is particularly useful for comparing the distribution of multiple groups that have very different sizes
# For example, it's hard to compare the distribution of price within cut because some groups are quite small. It's easier to compare if the groups are standardized to take up the same area: 
...
ggplot(mpg, aes(displ, hwy)) + geom_point() + geom_line(data = smoothed, aes(displ)) + geom_ribbon(data = smoothed, aes(ymin = hwy_lwr, ymax = hwy_upr), alpha = 0.2, fill = "red")
...
###############################################################################################################################################################################################
# Position adjustments apply minor tweaks to the position of elements within a layer. There are three adjustments apply primarily to bars:
# 1. position_stack() - Stack overlapping bars or areas on top of each other
# 2. position_fill() - Stack overlapping bars, scaling so the top is always at one
# 3. position_dodge() - Place overlapping bars (or boxplots) side-by-side

dplot <- ggplot(diamonds, aes(color, fill = cut)) + xlab(NULL) + ylab(NULL) + theme(legend.position = "none") # position stack is the default for bars so geom_bar() is equivalent to geom_bar(position = "stack")
...
# 3. position_ jitterdodge() - Dodge points within groups, then add a little random noise
